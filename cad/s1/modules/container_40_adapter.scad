include <../s1_parameters.scad>

build_part = "full";

module container_40_adapter_full() {
  union() {
    difference() {
      rounded_box([s1_module_length, s1_module_width, s1_module_interface_height], 6);
      mount_pin_holes(height = 14);
    }
    translate([0, 0, s1_module_interface_height - 0.3])
      rounded_box([s1_module_length - 18, s1_module_width - 10, 7], 3);
    translate([0, 0, s1_module_interface_height + 7])
      rounded_box([s1_module_length - 34, s1_module_width - 16, 34], 3);
    for (x = [-80, -40, 0, 40, 80])
      translate([x, s1_module_width / 2 - 10, s1_module_interface_height + 7])
        cube([1.4, 4, 32]);
    for (x = [-80, -40, 0, 40, 80])
      translate([x, -s1_module_width / 2 + 6, s1_module_interface_height + 7])
        cube([1.4, 4, 32]);
    cg_marker(48, 14);
  }
}

module container_40_adapter_part() {
  if (build_part == "front")
    difference() {
      render(convexity = 10) intersection() { container_40_adapter_full(); split_front_clip(s1_module_length, 110, 120); }
      split_alignment_sockets("front", z = 0.5);
    }
  else if (build_part == "rear")
    difference() {
      render(convexity = 10) intersection() { container_40_adapter_full(); split_rear_clip(s1_module_length, 110, 120); }
      split_alignment_sockets("rear", z = 0.5);
    }
  else
    container_40_adapter_full();
}

container_40_adapter_part();
